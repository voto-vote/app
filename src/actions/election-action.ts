...
export async function getElection(id: string): Promise<Election> {
    ...
    cacheLife("minutes")
    cacheTag(`election-${id}`)


    const availableLanguagesData = db.selectDistinct({ languageCode: statementTranslations.languageCode })
        .from(statementTranslations)
        .where(eq(statementTranslations.instanceId, parseInt(id)));

    const instanceData = db.select({
        id: instances.id,
    ...
    const configurationPromise = fetch(`https://votoprod.appspot.com.storage.googleapis.com/configuration/${id}/configuration.json`)
        .then(r => r.json());

    const [availableLanguages, instance, configuration] = await Promise.all([availableLanguagesData, instanceData, configurationPromise]);
