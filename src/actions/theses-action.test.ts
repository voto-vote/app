...
describe("parseThesisText", () => {
    it("replaces {title} and {location} placeholders", () => {
        const input = "Election: {title} in {location}";
        const result = parseThesisText(input, "General Election", "Berlin");
        expect(result.text).toBe("Election: General Election in Berlin");
        expect(result.explanations).toEqual([]);
    });

    it("extracts explanations and replaces text", () => {
        const input = "This is a (thesis)[explanation].";
        const result = parseThesisText(input, "Title", "Location");
    ...
    it("handles multiple explanations", () => {
    ...
    it("returns empty explanations if no matches", () => {
    ...
    it("trims explanation and thesis text", () => {
