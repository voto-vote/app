...
export default function Progress({ total, current, onChange }: ProgressProps) {
  ...
  useEffect(() => {
    ...
    // lastIndex = total - 1
    // middleIndex = lastIndex / 2
    // translationX = middleIndex - current * (width of a dot + dot spacing)
    setTranslateX(((total - 1) / 2 - current) * (dotWidth + dotSpacing));
  }, [total, current, isDragging, dotWidth]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: globalThis.MouseEvent) => {
      ...
      const newIndex = Math.round(
        Math.abs(newPosition - ((total - 1) * (dotWidth + dotSpacing)) / 2) /
          (dotWidth + dotSpacing)
      );
      if (newIndex !== current) {
        onChange(newIndex);
